...
def test_default_from_name() -> None:
...
def test_default_from_name_should_default_to_project_name() -> None:
    project_name = "PROJECT_NAME"
    config = Settings(PROJECT_NAME=project_name)
    assert config.DEFAULT_FROM_NAME == project_name


def test_emails_enabled_with_no_configuration_should_fail() -> None:
    emails_enabled = True
    smtp_host = ""
